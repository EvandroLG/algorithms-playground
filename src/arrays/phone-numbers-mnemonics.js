...
function phoneNumberHelper(idx, phoneNumber, tmp, output) {
  ...
  for (const letter of letters) {
    tmp[idx] = letter;
    phoneNumberHelper(idx + 1, phoneNumber, tmp, output);
  }
}

function phoneNumbersMnemonics(phoneNumber) {
  const output = [];
  const tmp = Array(phoneNumber.length).fill('-1');

  phoneNumberHelper(0, phoneNumber, tmp, output);

  return output;
}

const assert = require('assert');
assert.deepEqual(phoneNumbersMnemonics('1905'), [
  '1w0j',
  '1w0k',
  '1w0l',
  '1x0j',
