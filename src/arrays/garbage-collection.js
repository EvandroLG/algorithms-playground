/*
  ...
  Example:
  Input: ["G","P","GP","GG"], [2,4,3]
  Output: 21
*/

function garbageCollection(garbage, travel) {
  let result = 0;
  const trucks = getTrucks(garbage);

  for (const [truck, lastIdx] of Object.entries(trucks)) {
    for (let i = 0; i < garbage.length; i++) {
      result += removeGarbage(truck, garbage[i]);
...
function getTrucks(garbage) {
  const memo = {};

  for (let i = 0; i < garbage.length; i++) {
    const fragments = garbage[i].split('');
...
function removeGarbage(truck, garbage) {
  return garbage.split('').reduce((acc, g) => {
    if (g === truck) {
      acc++;
    }
