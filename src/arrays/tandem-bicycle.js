...
function tandemBicycle(redShirtSpeeds, blueShirtSpeeds, fastest) {
  if (fastest) {
    const copyRed = redShirtSpeeds.slice();
    const size = copyRed.length;
  ...
  const sortedRed = sort(redShirtSpeeds);
  const sortedBlue = sort(blueShirtSpeeds);

  return sortedRed.reduce((acc, cur, idx) => {
    const max = Math.max(cur, sortedBlue[idx]);
    return acc + max;
  }, 0);
}
...
assert.equal(tandemBicycle([5, 5, 3, 9, 2], [3, 6, 7, 2, 1], true), 32);
assert.equal(tandemBicycle([5, 5, 3, 9, 2], [3, 6, 7, 2, 1], false), 25);
